...
typedef RT_MODEL_approach_01_casadi_sqpmethod_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * approach_01_casadi_sqpmethod(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(approach_01_casadi_sqpmethod_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://approach_01_casadi_sqpmethod:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: approach_01_casadi_sqpmethod -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = approach_01_casadi_sqpmethod();
  if (rtmGetErrorStatus(S) != NULL) {
