...
% Solver options
% opts = struct('Osqp', struct('print_level', 0, 'sb', 'yes'), 'print_time', false);
% opti.solver('Osqp', opts);

opts = struct('qpsol', 'osqp');
...
%     {xinit, xref}, {u(:,1)}, ...
%     {'x0','xref'}, {'u_opt'});
F = opti.to_function('my_solver', ...
    {xinit, xref}, {u(:,1)}, ...
    {'x0','xref'}, {'u_opt'});


% Generate C code
% F.generate('my_solver.c', struct('with_header', true));
...
cg = CodeGenerator('my_solver_osqp',cg_options);
