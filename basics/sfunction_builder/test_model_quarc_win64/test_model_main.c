...
typedef RT_MODEL_test_model_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * test_model(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(test_model_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://test_model:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: test_model -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
